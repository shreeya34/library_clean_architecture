...
from modules.infrastructure.database.models.member import (
    BorrowedBooks,
    MemberLogins as MemberLoginsDB,
)
...
class MemberRepository(IMemberRepository):
    ...
    def get_available_books_by_title(self, db: Session, title: str) -> List[Book]:
        return db.query(Book).filter(Book.title == title, Book.stock > 0).all()

    def has_already_borrowed(self, db: Session, book_id: int, member_id: str) -> bool:
        return (
            db.query(BorrowedBooks)
            .filter(
                BorrowedBooks.book_id == book_id, BorrowedBooks.member_id == member_id
            )
            .first()
            is not None
        )
    ...
    def get_book_by_title(self, db: Session, title: str):
