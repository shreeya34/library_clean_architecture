...
class AdminService(AdminServiceInterface):
    ...
    def create_admin(self, admin: CreateModel, db: Session) -> dict:
        return CreateAdminUseCase(self.admin_repo).register_admin(db, admin)

    @db_exception_handler("login admin")
    def login_admin(self, admin_data: AdminLogins, db: Session) -> dict:
        return LoginAdminUseCase(self.admin_repo).login_admins(db, admin_data)
    ...
    def view_all_members(
        ...
        self._check_admin(current_user)
        return ViewMembersUseCase(self.admin_repo).view_members(
    ...
    def view_member_by_id(
        ...
        self._check_admin(current_user)
        return ViewMemberByIdUseCase(self.admin_repo).get_member_by_id(db, member_id)
